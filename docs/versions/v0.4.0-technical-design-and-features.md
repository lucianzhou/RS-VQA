...
- **模型选择器与滚动条 UI 优化**：
  1. 修复顶部模型选择器 Trigger 按钮的 Flex 布局：为名称与描述创建独立 `.model-selector-info` 容器，解除原本 selector 内所有 `span` 统一被 `flex: 1` 强制平分的异常挤压，消除文字被提前截断成 `...` 的问题。
  2. 修复全站滚动条入侵（Scrollbar Intrusion）重叠问题：全局引入 6px 纤细非侵入式 Mineral Forest 主题滚动条，并在 `.page-scroll` 和 `.conversation-canvas` 上配置 `scrollbar-gutter: stable`，确保滚动条不遮挡设置页中的下拉框、开关和页面卡片右侧边界。
